...
import '../components/article_tab_content.dart';
import '../components/home_tab_bar.dart';
import '../components/category_selector_dialog.dart';
...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  Future<void> _fetchCategories() async {
    try {
      final response = await http.get(
        Uri.parse('http://localhost:8000/api/categories/with-articles'),
      );
      if (response.statusCode == 200) {
        final data = json.decode(response.body) as List;
        final mainCategories =
          ...
          title:
              (!isLoading && _tabController != null)
                  ? HomeTabBar(
                    ...
                    onAddPressed: () => _showCategorySelector(context),
                  )
                  : null,
        ),
